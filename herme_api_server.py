...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        #added
        self.runtime_kwargs = _resolve_runtime_agent_kwargs()
        self.model = _resolve_gateway_model()
        user_config = _load_gateway_config()
        self.fallback_model = GatewayRunner._load_fallback_model()
        self.enabled_toolsets = sorted(_get_platform_tools(user_config, "api_server"))
        self.max_iterations = int(os.getenv("HERMES_MAX_ITERATIONS", "90"))
        
        self._agent_pool = Queue()
        self._session_agent_map: Dict[str, AIAgent] = {}
        self._agent_session_map: Dict[AIAgent, str] = {} 
        #idle agent put back
        ...
        self._mapping_lock = threading.RLock() # 锁保护映射表（因为可能在多个线程中访问）
        self._pool_size = 5
        for i in range(self._pool_size):
            agent = self._create_base_agent()
    ...
    def _get_agent_from_pool(self,session_id: str, 
                               ephemeral_system_prompt: Optional[str] = None,
                               stream_delta_callback=None,
                               tool_progress_callback=None,
                               tool_start_callback=None,
                               tool_complete_callback=None) -> Optional[AIAgent]:
        """
        从池中获取 agent（同步，非阻塞）
        返回 None 表示池满
        """
        with self._mapping_lock:
            if session_id in self._session_agent_map:
                agent = self._session_agent_map[session_id]
                return agent
            try:
                # get_nowait() 不等待，池空时立即抛出 Empty 异常
                agent = self._agent_pool.get_nowait()
                # 记录映射关系
                self._session_agent_map[session_id] = agent
                self._agent_session_map[agent] = session_id
    ...
    def _start_cleaner_thread(self):
        """启动后台线程，定期清理长时间未使用的 Agent"""
        def cleaner():
            while True:
                try:
                    time.sleep(60*5) # 每分钟检查一次
                    self._evict_idle_agents()
                except Exception as e:
                    print(f"Agent Pool Cleaner Error: {e}")
    ...
    def _evict_idle_agents(self):
        """
        清理逻辑：
        遍历池中的 Agent，如果闲置超过 30 分钟，则将其移除并销毁，
        同时补充新的 Agent 以保持池的最小大小（可选）。
        """
        current_time = time.time()
        
        with self._mapping_lock:
            # 遍历所有借出去的 Agent
            for session_id, agent in list(self._session_agent_map.items()):
                try:
                    # 1. 检查时间：是否超过了 30 分钟
                    # (这里继续使用 session_start，因为它代表了借用开始时间)
                    start_timestamp = agent.session_start.timestamp()
                    duration = current_time - start_timestamp
    
                    if duration > self.AGENT_MAX_LIVE_TIME:
                        
                        # 2. 检查状态：这是关键！
                        # 获取 Agent 的内部锁，检查它是否正在运行任务
                        with agent._task_status_lock:
                            if agent._is_running_task:
                                # --- 情况 A：任务还在跑 ---
                                continue 
                            else:
                                # --- 情况 B：任务卡住或已结束但未归还 ---
                                # 此时可以回收
                                self._return_agent_to_pool(session_id,agent)
    ...
    def _return_agent_to_pool(self,session_id, agent: AIAgent):
         with self._mapping_lock:
            # 会话结束，清理并归还
            if session_id in self._session_agent_map:
                del self._session_agent_map[session_id]
            if agent in self._agent_session_map:
                del self._agent_session_map[agent]
            
            # 重置 agent 状态
            self._reset_agent_sync(agent)
            self._agent_pool.put(agent)
    
    def _reset_agent_sync(self, agent: AIAgent):
        """重置 agent 状态（同步）"""
        agent._session_messages = []
        agent._api_call_count = 0
        agent._budget_exhausted_injected = False
        ...
        # 清空 tool worker threads
        with agent._tool_worker_threads_lock:
            agent._tool_worker_threads.clear()
    @staticmethod
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            agent=self._get_agent_from_pool(session_id, 
            ...
            try:
                ...
                with self._task_status_lock:
                    self._is_running_task = False
            #用户发出关闭某个session_id任务命令，经过模型，返回result中包含关闭特定session_id时：
            #当这个session等待时间超时，没有任何进一步访问的请求时关闭
            #_return_agent_to_pool(agent)
            usage = {
